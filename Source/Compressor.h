...
public:
    ...
    void process(int BufferSize)
    {
        ...
        if (la != nullptr)
            la_time = *la;
        delayBuffer.resize((int)(la_time * fs / 1000));
        grms = 0;

        // TIME COEFFS, *1000 bc of [ms]
        // all four coeffs are close, but not equal to 0
        float cat = 1 - (float)exp(-2.2 / fs / *at * 1000);
        float crt = 1 - (float)exp(-2.2 / fs / *rt * 1000);
        float rms_attack = 1 - (float)exp(-1 / fs / RMS_A_TIME * 1000);
        float rms_release = 1 - (float)exp(-1 / fs / RMS_R_TIME * 1000);
        ...
        for (int i = 0; i < BufferSize; i++) {
            ...
            float G = (1 - 1 / *CR) * (*CT - X);
            if (G > 0) G = 0;
            target = (float)pow(10, G / 20);          // current gain target
    ...
    void setfs(double SampleRate)
    {
        ...
        fs = SampleRate;
        delayBuffer.resize((int)(*la * fs / 1000));
    }
